...
const submitApplication = async () => {
  ...
  if (response.success) {
    let result = "";
    if (formData.value.loanAmount > response.loanAmount)
        result = "above";
    else if ( formData.value.loanAmount < response.loanAmount)
        result = "below";
    else
        result = "equal";
    
    toast.success("Application Saved Successfully. The new loan's loan amount is "+result+" the average.")
  } else {
...
<template>
  <div class="action-section">
    ...
    <BModal :visible="modal.isVisible.value" :confirm="modal.confirm">
      ...
      <form @submit.prevent="submitApplication()" id="loanApplicationForm">
      ...
      <template #footer>
        <BButton form="loanApplicationForm" type="submit" variant="primary" label="Submit"></BButton>
